...
The NuGet package bundles the entire `docs/` directory and a build-transitive target named `KF.Logging.docs.targets`. When a consuming project restores the package:
...
- To change the destination folder name, set the `KFLoggingDocsFolderName` MSBuild property in your project file:

  ```xml
  <PropertyGroup>
    <KFLoggingDocsFolderName>MyCompany.Logging</KFLoggingDocsFolderName>
  </PropertyGroup>
  ```
